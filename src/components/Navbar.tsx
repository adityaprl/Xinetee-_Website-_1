...
const Navbar = ({
  ...
  return <header className="sticky top-0 z-50 w-full border-b border-border/40 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container flex h-16 items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <Link to="/" className="flex items-center gap-2">
            ...
            <span className="hidden font-bold text-xl sm:inline-block">Xinetee</span>
          </Link>
        </div>
        ...
        <nav className="hidden md:flex items-center gap-6">
          ...
          <Link to="/verify" className={`text-sm font-medium transition-colors hover:text-primary ${location.pathname === "/verify" ? "text-primary" : "text-muted-foreground"}`}>
            Verify
          </Link>
        </nav>
