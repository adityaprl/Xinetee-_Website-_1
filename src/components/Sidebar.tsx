          ...
          <div className="h-full flex flex-col">
            ...
            <ScrollArea className="flex-1 border-t border-border/40">
              <div className="flex flex-col gap-1 p-2">
                {routes.map(route => <Link key={route.path} to={route.path} onClick={() => setIsOpen(false)}>
                    <Button variant={location.pathname === route.path ? "secondary" : "ghost"} className={cn("w-full justify-start gap-2", {
      ...
      <div className="hidden md:flex flex-col border-r border-border/40 w-16 transition-all duration-300 hover:w-56 group overflow-hidden">
        <div className="p-4 flex items-center justify-center md:justify-start">
          <Link to="/" className="flex items-center gap-2">
            ...
            <span className="font-bold text-xl hidden group-hover:inline-block whitespace-nowrap">Xinetee</span>
          </Link>
        </div>
        <nav className="flex-1 flex flex-col gap-1 p-2">
          {routes.map(route => <Link key={route.path} to={route.path}>
              <Button variant={location.pathname === route.path ? "secondary" : "ghost"} className={cn("w-full justify-start gap-2 overflow-hidden", {
            "bg-accent/50 text-accent-foreground": location.pathname === route.path
          })}>
        ...
        <div className="p-2 border-t border-border/40">
          <Button variant="ghost" className="w-full justify-start gap-2 text-muted-foreground overflow-hidden">
